    ...
    <div style={containerStyle}>
      ...
      <main style={mainStyle}>
        ...
        <div style={buttonContainerStyle}>
          ...
          <button style={buttonStyle} onClick={() => router.push("/admin")}>Back</button>
        </div>
                ...
                <div style={checklistStyle}>
                  {events.map((evt) => {
                    ...
                    return (
                      <span
                        key={evt.id}
                        className={`badge ${attended ? "attended" : "not-attended"}`}
                        title={attended ? "Attended" : "Not Attended"}
                      >
                        {evt.name}
                      </span>
                    );
      ...
      {/* Inline style for hover badges */}
      <style>{`
        .badge { padding: 6px 12px; border-radius: 20px; font-weight: 500; font-size: 14px; display: inline-block; transition: transform 0.15s ease, box-shadow 0.15s ease; cursor: default; }
        .badge.attended { background-color: #f4b400; color: white; }
        .badge.not-attended { background-color: #ccc; color: #555; }
        .badge:hover { transform: scale(1.05); box-shadow: 0 2px 6px rgba(0,0,0,0.2); }
      `}</style>
    </div>
  );
}
